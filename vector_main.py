...
def numba_run(position_array, velocity_array, acceleration_array, gravity_interaction_constant, infinite_diagonal, number_of_iterations=10**6, dt=1e6):
    ...
    position_saves = np.zeros((number_of_iterations + 1, n_bodies, 2))
    velocity_saves = np.zeros((number_of_iterations + 1, n_bodies, 2))
    ...
    for i in range(number_of_iterations):
        ...
        print(str(i+1) + "/" + str(number_of_iterations))
...
def orbit(a,e=0,varpi=None):
    if varpi is None:
        varpi = np.random.uniform(0, 2*np.pi)
    return (np.matmul(rotation_matrix(varpi), np.array([a*(1-e), 0])),
            np.matmul(rotation_matrix(varpi), np.array([0, np.sqrt(G_TIMES_MASS_SUN * (1+e) / (a * (1-e)))])))

def nice_model_objects():
    uranus_radius = np.random.uniform(11, 13)
    neptune_radius = np.random.uniform(max(uranus_radius + 2, 13.5), 17)
    radius_list = [5.45 * AU, 8.65 * AU, uranus_radius, neptune_radius]
...
def initialise_system(position_list, velocity_list, mass_list):
    ...
    position_array = np.zeros([n_bodies, 2])
    velocity_array = np.zeros([n_bodies, 2])
    mass_vector = np.zeros(n_bodies)

    position_sun = np.zeros(2)
    for i in range(n_bodies - 1):
        position_sun -= mass_list[i] * position_list[i] / MASS_SUN

    velocity_sun = np.zeros(2)
    for i in range(n_bodies - 1):
    ...
    gravity_interaction_constant = G * mass_vector[:, np.newaxis] * mass_vector[np.newaxis, :]
